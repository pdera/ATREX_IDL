...
pro locmax3, img, mtp, mask=m, where=w, ix=ix, iy=iy, sort=srt, $
  values=v, value_image=vimg, noedge=noed, help=hlp

  if (n_params(0) lt 1) or keyword_set(hlp) then begin
    print,' Find local 3-side maxima in an image.'
    print,' locmax, img'
    print,'   img = image to process.               in'
    print,' Keywords:'
    print,'   MASK=m returns a mask image with 1 at all'
  ...
  dy2 = shift(img,0,-1)
  ;------  compare each pixel to 4 surrounding values  -------
  m = ((img gt dx1+mtp) and (img gt dx2+mtp) and (img gt dy1+mtp)) or $ ;dy2
      ((img gt dx1+mtp) and (img gt dx2+mtp) and (img gt dy2+mtp)) or $ ;dy1
      ((img gt dx1+mtp) and (img gt dy1+mtp) and (img gt dy2+mtp)) or $ ;dx2
      ((img gt dx2+mtp) and (img gt dy1+mtp) and (img gt dy2+mtp))      ;dx1
