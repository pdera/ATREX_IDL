...
common calib_ref, zeroref
 ...
 ;   widget_control, wid_text_8, set_value=string(0, format='(I2)')+'/'+string(ni[0], format='(I2)')
    ...
    begin
     ...
     ;a1 is difference in intensity between locmax and surrounding pixels
...
begin
    ...
    begin
     ...
     begin
      ...
      print, '#peak candidates: ', n_elements(f1a)
      
      
      if f1a[0] ne -1 and n_elements(f1a) lt 5000 then $
      ...
      begin
        ...
        IF cgProgressBar -> CheckCancel() THEN BEGIN
          ...
          goto, eni
        ENDIF
        cgProgressBar -> Update, (100*(float(k)/n_elements(f1a)))        
      
        if oadetector->inside_detector_circle([IX[f[f1a[k]]],IY[f[f1a[k]]]], 12) then $
        begin
          ref_peak.selected[0]=0
          pic=oimage->get_zoomin([IX[f[f1a[k]]],IY[f[f1a[k]]]], [bx[0],bx[0]], maskarr)
          a=peak_candidate_evaluation(pic)
       
          if a[0] eq 1 then ref_peak.selected[0]=0 else ref_peak.selected[0]=1
  ...
  ;   opt->find_multiple_peak_copies
    ...
    if opt->peakno() eq 0 then rf=dialog_message('Too many peaks have been detected. Please adjust search parameters'); all peaks
    eni:
    return, opt->peakno()
end
;--------------------------------------------------
