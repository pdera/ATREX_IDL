...
COMMON WID_MAR345_elements
     ...
     if erasea eq 1 then $
     plot_image, oimage
     oadetector->change_caltype, read_cal_type()
     path=oadetector->generate_ds_ring(tth, kt)
...
'TDS':$
begin
 SIZ=read_box_size(text1, text2)
 siz=siz[0]
 ad=oadetector->get_object()
 xy0=[ad.beamX,ad.beamY]
 ...
 begin
    ...
    if 	(detXY[0]-siz) lt 0 or (detXY[0]+siz) gt arr1 or $
        (detXY[1]-siz) lt 0 or (detXY[1]+siz) gt arr2 then $
    begin
      re=dialog_message('Peak too close to the image edge')
    endif else $
    begin
	fi=dialog_pickfile(/write, filter='*.txt', default_extension='txt', path=dir)
	if fi ne '' then $
	begin
	  get_lun, lu
	  openw, lu, fi
      ...
      for x=-siz, siz do $
	     ...
	     begin
	       ...
	       dx0=dx
	       ; invert y with respect to beam center
	       dry=dx[1]-xy0[1]
	       drx=dx[0]-xy0[0]
	       ;dx[0]=xy0[0]-dry
	       dx[1]=xy0[1]-dry
           ...
           ref_peak.gonio=[an[0],an[1],-g[2:5]]
           ref_peak.xyz=oadetector->calculate_XYZ_from_pixels_mono(dx, ref_peak.gonio, wv)
           hkl=hkl_from_ub_and_xyz(ub, ref_peak.xyz)
	       printf, lu, long(dx), hkl,  im.img[dx0[0],dx0[1]], format='(I6, I6, F8.4, F8.4,  F8.4,  F15.2)'
	     endfor
	     close, lu
	  	 free_lun, lu
...
;--------------------------------------
  ...
  'Test calibration1':$ ; This is something for spec files
  begin
    fn=dialog_pickfile(/read, filter='*.dat',  path=out_dir)
    dn=read_spec_dat(fn)
    print, dn
    ktth=read_kappa_and_ttheta()
    Dcenter=ktth
    print,Dcenter ;nu and del
    ad=oadetector->get_object()
    x0=ad.beamx
    y0=ad.beamy
    ref_peak.detXY=[x0,y0]
    ref_peak.gonio=fltarr(6)
    ref_peak.gonio[0]=ktth[0]
    ref_peak.gonio[1]=ktth[1]
    ref_peak.selected[0]=1
   ...
   begin
     ...
     kt=dn[0:1,i]
     ref_peak=ref_peak0
     ref_peak.gonio[0]=kt[0]
     ref_peak.gonio[1]=kt[1]
     ref_peak.detXY=oadetector->calculate_pixels_from_xyz(ref_peak.xyz, ref_peak.gonio)
     opt->appendpeak, ref_peak
   endfor
       plot_image, oimage
       plot_peaks, draw0, opt, arr1, arr2
       print_peak_list, opt, wid_list_3, oadetector
  ...
  'Import abs':$
     ...
     ;on_IOERROR, en
      ...
      begin
       ...
       close, lu
       wset, wid_draw_6
       a=generate_abs(min(tab[*,0]),max(tab[*,0]))
       s=n_elements(a)/2
       device, decomposed=1
...
begin
 ...
 begin
  ...
  begin
   ...
   prop=is_sequence_number_proper(fn)
   extno=how_many_digits(fn)
   if prop eq -1 then extno=-1
...
'PS Series':$; ---- Peak search for series of images
 begin
   if test_write(dir) eq 1 and test_write(out_dir) eq 1 then $
   begin
    lp=PS_Series()
    if lp[0] eq 0 then merge_peak_tables_in_series
   endif else re=dialog_message('Either image or ourtput directory does not have write permission')
 end
...
'Open PT':$
 begin
   fn=dialog_pickfile(/read, filter=['*.pks', '*.spc'], path=out_dir)
   rr=file_info(fn)
   if fn ne '' and rr.exists eq 1 then $
   begin
      ln=strlen(fn)
      ext=strmid(fn,3,4, /REVERSE_OFFSET)
      if ext eq '.spc' then $
      begin
         opt->import_spec_angles, wv, fn, ub, oadetector
      end else $
      begin
      ...
      print_R_int, Rint(get_laue_class(), opt)
      peaktable_file=fn
   endif else re=dialog_message('Selected file does not exist')
 end
...
'Draw3':$
 ...
 'Draw':$
 begin
  if abs(total(ub)) gt 0.0 then $
  begin
   wset, DRAW0
   cursor, x, y, /device, /nowait
   detxy=[x*float(arr1/600.),y*float(arr2/600.)]
   ...
   ref_peak.detXY=detXY
   g=read_gonio()
   an=read_kappa_and_ttheta()

   ref_peak.gonio=[an[0],an[1],-g[2:5]]
   ref_peak.xyz=oadetector->calculate_XYZ_from_pixels_mono(ref_peak.detXY, ref_peak.gonio, wv)
   ;ref_peak.xyz[2]=-ref_peak.xyz[2]
   hkl=hkl_from_ub_and_xyz(ub, ref_peak.xyz)
   ;ref_peak.hkl[0]=-ref_peak.hkl[0]
   ref_peak.hkl=hkl
   update_hkl, hkl
  endif
...
'Open gonio':$
  begin
    print, 'open gonio'
    fn=dialog_pickfile(/read, path=out_dir, filter='*.gonio')
    if fn ne '' then gonio=read_gonio_from_file(fn)
    print_gonio, gonio
  end
...
'Save gonio' :$
  begin
    print, 'save gonio'
    gonio=read_gonio()
    fn=dialog_pickfile(/write, path=out_dir, filter='*.gonio', default_extension='gonio')
    if fn ne '' then save_gonio, gonio, fn
  end
'Clear gonio' :$
  begin
    print, 'clear gonio'
...
;----------------- read intentisy scale
 ...
 begin
   ...
   pic9=one_peak_get_profile(pn,bs)
  ...
  ; endif
   ...
   ;---------------------------------------------------------------------------------------------
  print, '==='
  print, peak_candidate_evaluation(pic)  
  print, '==='

       pic22=pic-pic9

       m1=max(pic22)   ; maximum difference
       m1n=min(pic22)   ; maximum difference
       m3=median(pic)
       ...
       c1=(m6-median(pic))/median(pic);'  maximum int in pic above median/median =       '
       c2=(stf-stp)/stf;'  normallized difference in total intensity 7x7 center = '
       c3=total(abs(spic22>0))/stf;print, '----- ', pn, '  normallized total positive difference center          =', 

          print, '------------------------ '
          print, '----- ', pn, '  maximum int in pic above median/median =       ', (m6-median(pic))/median(pic)
          print, '----- ', pn, '  total int in pic above median/median   =       ', tf/(median(pic))
          print, '----- ', pn, '  max positive difference/max int in pic= ', m1/(m6-median(pic))
          print, '----- ', pn, '  max negative difference/max int in pic= ', m1n/(m6-median(pic))
          print, '----- ', pn, '  difference in median=                          ', (m3-median(pic9))/m3
          print, '----- ', pn, '  normalized difference in total intensity above median= ', (tf-tp)/tf
          print, '----- ', pn, '  normallized difference in total intensity 7x7 center = ', (stf-stp)/stf
          print, '----- ', pn, '  normallized total positive difference center          =', long(total(abs(spic22>0)))/stf
          print, '------------------------ '
          
    if c1 lt 1.0 then print, 'weak peak' 
    if c1 lt 0.2 and tf/(median(pic)) lt 2.0 then print, 'Too weak peak'
    if c2 gt 0.25 then print, 'Poor total fit at the center'
    if c3 gt 0.25 then print, 'Poor detailed fit at the center'
...
selecting_status=0
 ...
 WIDGET_CONTROL, WID_TEXT_0a, EDITABLE=1
  WIDGET_CONTROL, WID_TEXT_0a, SET_VALUE='2'
...
;peak filtering
 ...
 WIDGET_CONTROL, WID_BUTTON_49_1, set_uvalue='Open gonio'
 WIDGET_CONTROL, WID_BUTTON_49_2, set_uvalue='Save gonio'
 WIDGET_CONTROL, WID_BUTTON_49_3, set_uvalue='Clear gonio'
