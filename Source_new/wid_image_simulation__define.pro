...
begin
  ...
  rbc=read_box_change()
  oadetector->generate_all_peaks, ub, opt, wv, pred, brav, DAC_open, read_box_size(), kt, rbc
  bx=read_box_size()
  ...
  print_peak_list, opt, wid_list_3a, oadetector, wv
...
;--------------------
   ...
   begin
     ...
     begin
        ...
        self->print_UB, UB
        self->print_lp, lp
        if n_elements(Cij_window) ne 0 then $
        if Cij_window ne -1 then $
        begin
           cubic=widget_info(Cij_window, find_by_uname='WID_BUTTON_ref_cubic')
           iscubic=widget_info(cubic, /button_set)
           if iscubic then $
           begin
        		V0=V_from_UB(ub)
    			a0=V0^(1.0/3)
    			lp_ref=[a0,a0,a0,90.,90.,90.]
    			list_ref=widget_info(Cij_window, find_by_uname='WID_LIST_reference')
   ...
   self.widgets.wid_button_10e:$;       'Open Cij'
   begin
       if abs(total(ub)) gt 0.0 then wid_cij else re=dialog_message('Read ub matrix first')
...
;        print, calculate_stress(ub)
;     end
   end

   self.widgets.wid_button_10f:$;       '/2pi and transposition'
   begin
        ...
        xyz=xyz_from_angles_6circle(test_angles(), 0.4275)
        print, 'hkls @@@@@@@@@@@'
        print, invert(UB) ## xyz
...
function WID_Image_simulation::init
  ...
  self.widgets.WID_Image_simulation = Widget_Base(UNAME='WID_Image_simulation' ,XOFFSET=5 ,YOFFSET=5  $
      ,SCR_XSIZE=640 ,SCR_YSIZE=487 ,TITLE='Diffraction image'+ $
  ...
  self.widgets.WID_BUTTON_10e = Widget_Button(self.widgets.WID_Image_simulation,  $
      UNAME='WID_BUTTON_10e' ,XOFFSET=510 ,YOFFSET=190 ,/ALIGN_CENTER  $
      ,VALUE='Cij')

  self.widgets.WID_BUTTON_10f = Widget_Button(self.widgets.WID_Image_simulation,  $
      UNAME='WID_BUTTON_10f' ,XOFFSET=553 ,YOFFSET=190 ,/ALIGN_CENTER  $
      ,VALUE='/2pi')
