...
;---------------------------------
pro print_peak_list, opt, list, oad, wv

 COMMON WID_peakfit2d_elements
 pt=opt->get_object()
 if pt.peakno eq 0 then tx='' else $
 begin
  nu=oad->get_nu_from_xyz(pt.peaks[0].DetXY)
  tth=get_tth_from_xyz(pt.peaks[0].xyz, wv)
  ...
  begin
    nu=oad->get_nu_from_xyz(pt.peaks[0].DetXY)
    tth=get_tth_from_xyz(pt.peaks[i].xyz, wv)
