...
; Caculates the diffracted beam unit vector coordinates
; from the reciprocal vector coordinates
; Checked 03/04/05
; hl is at diffracting position
; used in adetector_class::calculate_pixels_from_xyz
...
; from diffracted beam unit vector (hl) coordinates
; Checked 03/04/05
...
return, [6.4765,    8.9484,    0.0112, -146.0005,    0.4970,    0.2477]
;return, [6.4073, 12.5934, 19.8566, -173.764, 0.0035, 0] ; 0 -2 1
 ; lambda =0.4275
end
...
function xyz_from_angles_6circle, angles, lambda
 ; Eulerian angle definitions
 ; omega and phi are R3
 ; psi circle is  R2
...
common Rota, Mtx
  ...
  Sxyz0=[1,0,0]
  Sxyz1= Dnu ## Ddel ## Sxyz0 ; scattered beam position

  Rxyz=(Sxyz1-Sxyz0)/lambda[0] ; reciprocal vector in scattering position
  ...
  sampleR= phi ## chi ## ome ## psi

  xyz=sampleR ## Rxyz ; reciprocal vector at zero position
...
function Elasticity_pyrope, p

; right now p is number of pressure point in Conrad et al. (1999) Amer. Mineral 84, 374-383
...
1: Cs=[0.00, 297.6, 109.8, 92.7]
2: Cs=[3.38, 313.8, 119.8, 94.8]
3: Cs=[6.53, 328.7, 122.8, 101.5]
4: Cs=[8.75, 339.7, 133.6, 103.1]
...
function get_nu_from_pix, pix, pix0, wv
;--- on Jan 03, 2011 this function is being replaced by a class method with the same name
...
;--------------------------------------
function get_tth_from_xyz, xyz, wv
...
;calculates unit cell volume from cell parameters
 B=B_from_lp(lp)
 gs=transpose(b)##b
 g=invert(gs)
...
;------------------------------------------
function calculate_strain, ub, lp_ref

;NOTES:

; * UB should be refined against d-spacings and with triclinic constraints
; * for now this function only works in cubic symmetry

;--2.	Produce a B matrix that corresponds to this UB and store information about U

 B=B_from_ub(ub)
 U=U_from_ub(ub)

;--3.	Calculate corresponding A1 matrix by inverting B.

 A1=invert(B)

;--4.	Calculate reference state A0 matrix, with the same volume as A, but assuming ideal symmetry.
;       According to this reference state the stress we will determine is purely deviatoric (no volume change)
...
;--5.	Calculate strain mtx Eps, from eps1 = (A1-A0)A0^-1

 eps1= (A1-A0) # invert(A0)

;--6.	Calculate strain w/o rotations eps =(eps1^T+ eps1)/2

 eps= (transpose(eps1)+eps1)/2.0

 eps1= U ## eps ##  transpose(U)
...
;4: 8.75 GPa
 ...
 Cs1=Elasticity_pyrope(n)
 Cs=Cs1[1:3]
 Cij=Cij_from_Cs_cubic(Cs)
 print, cij

end
;-----------------------------------------

function calculate_stress, ub, cij, lp_ref
...
;--7.	Convert to Voigt vector notation

 epsV=matrix_to_Voigt(eps)

;--8.	Use Hooke's law to calculate stress   (in Voigt notation)

; Cs1=Elasticity_pyrope(4)
; Cs=Cs1[1:3]
; Cij=Cij_from_Cs_cubic(Cs)
 sigV1=Cij # epsV

;--9.	Convert stress   to matrix notation

sig1=Voigt_to_matrix(sigV1)

;--10.	Apply U matrix to rotate the stress tensor from crystal to laboratory reference system.

sig= (U) ## sig1 ## transpose(U)

return, sig
;return, (transpose(sig) + sig)/2.0
